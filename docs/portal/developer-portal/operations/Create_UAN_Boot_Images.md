...
9. Apply any site-specific customizations and modifications to the Ansible configuration for the UAN nodes and commit the changes.
    ...
    **Warning:** Never place sensitive information such as passwords in the git repository.
    ...
    These and other Ansible files do not necessarily need to be modified for UAN image creation. See [About UAN Configuration](operations/About_UAN_Configuration.md#about-uan-configuration) for instructions for site-specific UAN configuration, including CAN/CHN configuration.  See [Configure Interfaces on UANs](Configure_Interfaces_on_UANs.md) for details on CAN/CHN configuration.
